...
export function ProjectGrid(){
  const filters=["All","Data Analytics","Machine Learning","Generative AI","Computer Vision","Healthcare","Healthcare Analytics","Fraud Analytics","Environmental and Water Analysis","Deployed Apps"];
  const [active,setActive]=useState("All");
  const visible=active==="All"?projects:projects.filter(p=>p.filters.includes(active));
  const tilt=(e:React.MouseEvent<HTMLElement>)=>{const el=e.currentTarget,r=el.getBoundingClientRect(),px=e.clientX-r.left,py=e.clientY-r.top,x=px/r.width-.5,y=py/r.height-.5;el.style.setProperty("--rx",`${-y*7}deg`);el.style.setProperty("--ry",`${x*9}deg`);el.style.setProperty("--mx",`${px}px`);el.style.setProperty("--my",`${py}px`)};
  ...
  return <><div className="project-filters" role="group" aria-label="Filter projects">{filters.map(f=><button key={f} type="button" className={active===f?"active":""} aria-pressed={active===f} onClick={()=>setActive(f)}>{f}<span>{f==="All"?projects.length:projects.filter(p=>p.filters.includes(f)).length}</span></button>)}</div><div className="project-grid" aria-live="polite">{visible.map((p,i)=><Link href={`/projects/${p.slug}`} key={p.slug} className={`project-card card-${projects.indexOf(p)+1} ${i===0?"wide":""}`} onMouseMove={tilt} onMouseLeave={reset}><div className={`project-visual ${p.accent}`}><div className="cursor-glow"/><span className="project-number">{p.index}</span>{p.image?<img className="demo-capture" src={p.image} alt={`${p.title} live Hugging Face interface`}/>:<><div className="visual-grid"/><ProjectArt slug={p.slug}/></>}<span className="visual-label">VIEW CASE STUDY</span><span className="open-mark">↗</span></div><div className="project-info"><p>{p.category}</p><h3>{p.title}<i className="project-arrow">↗</i></h3><span>{p.summary}</span></div></Link>)}</div>{visible.length===0&&<div className="project-empty"><span>COMING SOON</span><p>Projects in this area are being developed and will be added here.</p></div>}</>;
}
