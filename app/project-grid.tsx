...
export function ProjectGrid({featured=false}:{featured?:boolean}){
  const filters=["All","Data Analytics","Machine Learning","Generative AI","Computer Vision","Healthcare","Deployed Apps"];
  const source=featured?projects.slice(0,4):projects;
  const [active,setActive]=useState("All");
  const visible=active==="All"?source:source.filter(p=>p.filters.includes(active));
  ...
  return <>{!featured&&<><div className="filter-heading"><span>EXPLORE BY FOCUS</span><p>Browse the complete body of work by discipline.</p></div><div className="project-filters" role="group" aria-label="Filter projects">{filters.map(f=><button key={f} type="button" className={active===f?"active":""} aria-pressed={active===f} onClick={()=>setActive(f)}>{f}<span>{f==="All"?projects.length:projects.filter(p=>p.filters.includes(f)).length}</span></button>)}</div></>}<div className="project-grid" aria-live="polite">{visible.map((p,i)=><Link href={`/projects/${p.slug}`} key={p.slug} className={`project-card card-${projects.indexOf(p)+1} ${i===0?"wide":""}`} onMouseMove={tilt} onMouseLeave={reset}><div className={`project-visual ${p.accent}`}><div className="cursor-glow"/><span className="project-number">{p.index}</span>{p.image?<img className="demo-capture" src={p.image} alt={`${p.title} live Hugging Face interface`}/>:<><div className="visual-grid"/><ProjectArt slug={p.slug}/></>}<span className="visual-label">VIEW CASE STUDY</span><span className="open-mark">↗</span></div><div className="project-info"><p>{p.category}</p><h3>{p.title}<i className="project-arrow">↗</i></h3><span>{p.summary}</span></div></Link>)}</div></>;
}
